...
class PostItem {
  ...
  List<articleModel> getPostList() {
    return postList;
  }

  articleModel getPostItembyIdx(int value) {
    return postList[value];
  }

  void addPostItem(articleModel post) {
    postList.add(post);
  }
  ...
  Future<void> addLoadPostItem() async {
    ...
    List<articleModel> newList = avoidDuplication(list);
  ...
  List<articleModel> avoidDuplication(List<dynamic> newlist) {
    List<articleModel> res = [];

    var currentArticleIds = (postList.asMap().entries.map((entry) {
      MapEntry<int, articleModel> map = entry;
      return map.value.id;
    })); //.contains('04');

    List<articleModel> list = newlist;
    for (int i = 0; i < newlist.length; i++) {
